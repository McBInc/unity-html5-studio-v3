...
export async function applyDiscordBasicPatch(
    zipBuffer: Buffer,
    hudOverlayJs: string
): Promise<{ patchedZip: Buffer; patch: { ok: boolean; injectedHud: boolean } }> {
...
export async function applyDiscordProducerPatch(
    zipBuffer: Buffer,
    init2026Js: string,
    commerceJslib: string
): Promise<{ patchedZip: Buffer; patch: { ok: boolean; injectedInit: boolean; wroteCommerce: boolean } }> {
