 ...
 */
export async function scanWebglBuildZip(zipBuffer, platformTarget) {
    ...
    try {
        ...
        if (!buildDir) {
            throw new Error("Could not locate a Unity WebGL Build folder (missing .data/.wasm).");
        }
        ...
        const deployable = structure.has_build_dir &&
            structure.has_data &&
            structure.has_wasm &&
        ...
        let metaComplianceResult = undefined;
        let discordComplianceResult = undefined;
        let youtubeComplianceResult = undefined;
        let tiktokComplianceResult = undefined;
        let linkedinComplianceResult = undefined;
        let telegramComplianceResult = undefined;
        ...
        const report = {
            ...
            compression: {
                ...
                notes: "Detected by .br/.gz artifacts in the Build folder; compression_mode summarizes the full state.",
            },
...
async function detectStructure(projectRoot, buildDir) {
    const notes = [];
    ...
    const hasFramework = lower.some((n) => n.includes(".framework.js") || n.endsWith(".framework.js.br") || n.endsWith(".framework.js.gz"));
    const hasData = buildEntries.some((n) => n.endsWith(".data") || n.endsWith(".data.br") || n.endsWith(".data.gz"));
    const hasWasm = buildEntries.some((n) => n.endsWith(".wasm") || n.endsWith(".wasm.br") || n.endsWith(".wasm.gz"));
    if (!hasIndexHtml)
        notes.push("Missing index.html at project root (next to Build/).");
    if (!hasTemplateData)
        notes.push("Missing TemplateData/ folder at project root (next to Build/).");
    if (!hasLoader)
        notes.push("Missing *.loader.js in Build/ folder.");
    if (!hasFramework)
        notes.push("Missing framework.js (or compressed variant) in Build/ folder.");
    return {
...
function computeCompressionMode(files) {
...
function computeQuickScore(opts) {
    ...
    const structurePoints = (s.has_index_html ? 8 : 0) +
    ...
    if (opts.compressionMode === "brotli")
        score += 10;
    if (opts.compressionMode === "gzip")
        score += 6;
    if (opts.compressionMode === "mixed")
        score += 2; // mixed tends to cause confusion
    // Clamp
...
function pickRecommendedHost(compat) {
    // Prefer Netlify first if compatible; then Vercel; then Nginx/Apache; then GitHub Pages (only for uncompressed).
    if (compat.netlify.compatible)
        return "netlify";
    if (compat.vercel.compatible)
        return "vercel";
    if (compat.nginx.compatible)
        return "nginx";
    if (compat.apache.compatible)
        return "apache";
    if (compat.github_pages.compatible)
        return "github_pages";
    return "netlify"; // default fallback
}
async function extractMemoryHints(loaderPath) {
