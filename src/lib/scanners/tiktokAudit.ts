...
export interface TiktokComplianceResult {
    platform: "TIKTOK";
    auditDate: string;
    score: number;
    criticalFailures: {
    ...
    checks: {
        touch_action_locked: boolean;
    };
}

export async function auditTiktokCompliance(
    tmpRoot: string,
    buildDir: string,
    projectRoot: string
): Promise<TiktokComplianceResult> {
    const result: TiktokComplianceResult = {
        platform: "TIKTOK",
    ...
    try {
        ...
        const scanForTouchLogic = async (dirPath: string) => {
