...
async function readAssets() {
    const read = async (p: string) => await fs.readFile(path.join(process.cwd(), p), "utf-8");
    return {
...
async function runPatchValidator() {
    ...
    const platforms = ["META", "DISCORD", "TIKTOK", "LINKEDIN_GAMES", "TELEGRAM"] as const;
    ...
    for (const platform of platforms) {
        ...
        // 2. Patch the ZIP
        let patchedBuffer: Buffer = rawZipBuffer;
